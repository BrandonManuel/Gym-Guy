...
var init_timer_value: float = 5.0
...
func setupLetters():
	...
	if score >= 25:
		if timer_value > .5:
			timer_value = init_timer_value - (.25 * (score - 25))
			tween.tween_property(progress_bar, "value", 0, timer_value)
...
func _input(event: InputEvent) -> void:
	if roundStarted and event is InputEventKey:
		if not is_letter(event):
			return
...
func is_letter(event: InputEvent) -> bool:
